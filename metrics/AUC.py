...
class AUC:
    def __init__(self, plot_ROC=False, file_path=None, verbose=1):
        self.plot_ROC = plot_ROC
        self.file_path = file_path
        self.verbose = verbose

    def extend_postive_range(self, x, window=16):
        label = x.copy().astype(float)
        # log(f"{label}", verbose=self.verbose, file_path=self.file_path)
        L = range_convers_new(label)  # index of non-zero segments
        # log(f"{L}", verbose=self.verbose, file_path=self.file_path)
        length = len(label)
    ...
    def TPR_FPR_RangeAUC(self, labels, pred, P, L):
        ...
        # log(f'recall {str(recall)}', verbose=self.verbose, file_path=self.file_path)
        ...
        # log(f"{existence_ratio}", verbose=self.verbose, file_path=self.file_path)
    ...
    def Range_AUC(self, score_t_test, y_test,  window=5, percentage=0, AUC_type='window'):
        ...
        P = np.sum(labels)
        # log(f"{np.sum(labels)}", verbose=self.verbose, file_path=self.file_path)
        if AUC_type == 'window':
        ...
        for i in np.linspace(0, len(score) - 1, 250).astype(int):
            ...
            # log(f"thre={str(threshold)}", verbose=self.verbose, file_path=self.file_path)
    ...
    def point_wise_AUC(self, score_t_test, y_test):
        # area under curve
        label = y_test
        score = score_t_test
    ...
    def __call__(self, score_t_test, y_test,  window=5, percentage=0, AUC_type='window'):
        point_auc = self.point_wise_AUC(score_t_test, y_test, window, percentage, AUC_type)
        range_auc = self.Range_AUC(score_t_test, y_test)
...
def main():
    ...
    auc = AUC(verbose=1,plot_ROC=False)
    point_auc = auc.point_wise_AUC(pred_labels, y_test)
    range_auc = auc.Range_AUC(pred_labels, y_test)
    print(f"point_auc: {point_auc}, range_auc: {range_auc}")
