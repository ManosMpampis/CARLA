...
class TSRepository(object):
    def __init__(self, n, dim, use_fneighbors=False):
        self.n = n if use_fneighbors else n*2
        self.dim = dim 
    ...
    def furthest_nearest_neighbors(self, topk, use_original_algorithm=False, memory_efficient=False):
        ...
        if use_original_algorithm:
            ...
            if not self.use_fneighbors:
                ...
                # Faster but very heavy on memory
                    ...
                    k_nearest_neighbours = torch.topk(dist, topk+1, largest=False).indices.cpu().numpy()[:, 1:]
                    k_furthest_neighbours = torch.topk(dist, topk).indices.cpu().numpy()
                ...
                if memory_efficient:
                    ...
                    for feature in self.features:
                        ...
                        # Bottomk but use the same variable for memory efficiency
    ...
    @torch.no_grad()
    def fill_ts_repository(self, p, loader, model, real_aug=False):
        """_summary_
        ...
        Args:
            ...
            real_aug (bool, optional): Determines if the dataloade contained injected augmentations and save the new values to the Time Series Repositorys. Defaults to False.
        """
        ...
        con_fneighbors = torch.tensor([], device="cpu") if self.use_fneighbors else None
        con_f_target = torch.tensor([], device="cpu") if self.use_fneighbors else None
